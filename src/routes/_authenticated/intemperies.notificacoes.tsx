...
function NotificacoesPage() {
  ...
  return (
    <ModuleShell
      icon={Bell}
      title="Notificações · Diagnóstico"
      subtitle="Configure Push, teste seu telefone e veja o histórico de entregas."
      status="operacional"
    >
      <div className="space-y-4 p-3 sm:p-4">
        ...
        <div className="rounded-xl border border-border bg-black/30 p-4">
          ...
          {!canUsePush() && (
            <div className="mt-2 text-[11px] text-amber-300">
              Push não está disponível no preview do Lovable ou fora de HTTPS. Abra a URL publicada
              no seu telefone.
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-border bg-black/30 p-4 space-y-2">
          ...
          <DiagRow
            ...
            ok={diag.notification}
            detail={typeof Notification !== "undefined" ? Notification.permission : ""}
          />
        ...
        <div className="rounded-xl border border-border bg-black/30 p-4">
          ...
          {subs.isLoading ? (
            ...
            <div className="text-xs text-muted-foreground">
              Nenhum dispositivo. Clique em "Ativar alertas de raios" acima.
            </div>
          ) : (
              ...
              {subs.data!.map((s) => (
                ...
                >
                  ...
                  <div className="mt-1 flex flex-wrap gap-3 text-[10px] text-muted-foreground">
                    ...
                    {s.last_success_at && (
                      <span>
                        Último sucesso: {new Date(s.last_success_at).toLocaleString("pt-BR")}
                      </span>
                    )}
                    {s.last_failure_at && (
                      <span>
                        Última falha: {new Date(s.last_failure_at).toLocaleString("pt-BR")}
                      </span>
                    )}
          ...
          {delivs.isLoading ? (
            ...
            <div className="space-y-1 text-xs">
              {delivs.data!.map((d) => (
                <div
                  key={d.id}
                  className="flex items-start justify-between rounded-md border border-border bg-background/40 px-2 py-1"
                >
                  <div>
                    <div className="font-medium">
                      {d.title}{" "}
                      {d.is_test && (
                        <span className="ml-1 rounded bg-amber-500/20 px-1 text-[9px] text-amber-300">
                          TESTE
                        </span>
                      )}
                    </div>
                    <div className="text-[10px] text-muted-foreground">
                      {new Date(d.created_at).toLocaleString("pt-BR")} · {d.severity ?? "-"} · HTTP{" "}
                      {d.http_status ?? "-"}
                    </div>
                    {d.error && <div className="text-[10px] text-red-300">{d.error}</div>}
                  </div>
                  <div
                    className={`text-[10px] font-bold ${d.status === "sent" ? "text-emerald-400" : "text-red-400"}`}
                  >
                    {d.status}
                  </div>
                </div>
        ...
        <div className="rounded-xl border border-border bg-amber-500/5 p-3 text-[11px] text-amber-200/90">
          <b>Importante:</b> vibração e som personalizados dependem do sistema operacional do
          celular. Em iOS, notificações web push só chegam quando o app está{" "}
          <b>instalado como PWA</b> (Adicionar à Tela Inicial). Em Android, funcionam com o
          navegador aberto ou fechado, desde que a permissão tenha sido concedida.
        </div>
      </div>
    </ModuleShell>
  );
...
export const Route = createFileRoute("/_authenticated/intemperies/notificacoes")({
  ...
  head: () => ({
    meta: [
      { title: "Notificações · Diagnóstico · VisionGuard AI" },
      {
        name: "description",
        content: "Configuração e diagnóstico de notificações Push para alertas de raios e clima.",
      },
    ],
  }),
