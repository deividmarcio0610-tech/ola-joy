...
import {
  Send, Search, RefreshCw, CheckCircle2, XCircle, Clock, AlertTriangle,
  FileCheck2, History as HistoryIcon, Filter, Download, FileText, Sparkles,
  Layers, TrendingUp, Timer, RotateCcw, GitBranch, X,
} from "lucide-react";
import {
...
import {
  listMonitoring, confirmValeSend, updateValeStatus, listHistory,
  registerValeResult, createValeRevision, listValeVersions, generateValeInsights,
...
export const Route = createFileRoute("/_authenticated/monitoramento-vale")({
  head: () => ({
    meta: [
      { title: "Monitoramento Vale · VALETECH" },
      { name: "description", content: "Controle de eventos enviados, resultados, aprovações, reprovações e tratativas com a Vale." },
    ],
  }),
...
const REJECT_CATEGORIES = [
  "Foto insuficiente", "Evidência inadequada", "Descrição incompleta",
  "Classificação incorreta", "Registro duplicado", "Ação proposta insuficiente",
  "Plano de ação incompleto", "Responsável não definido", "Prazo inadequado",
  "Ausência de comprovante", "Dados divergentes", "Registro fora do padrão",
  "Condição não caracterizada", "Solução tecnicamente inadequada", "Outro",
];
const CHANNELS = ["IA da Vale", "Sistema interno Vale", "E-mail", "WhatsApp corporativo", "Teams", "Outro"];
const NEON = "#39ff14";
const CHART_COLORS = [NEON, "#3b82f6", "#eab308", "#ef4444", "#f97316", "#a855f7", "#06b6d4", "#84cc16"];
...
const emptyFilters: Filters = {
  from: "", to: "", module: "all", status: "all", result: "all",
  area: "", priority: "all", q: "", reject: "all",
  onlyOverdue: false, onlyNoProtocol: false,
};
...
function MonitoringPage() {
  ...
  const filtered = useMemo(() => {
    return records.filter((r) => {
      ...
      if (filters.reject !== "all" && (r.vale_reject_category ?? "") !== filters.reject) return false;
      ...
      if (filters.q) {
        ...
        const hay = [r.internal_code, r.vale_protocol, r.vale_code, r.title, r.description, r.area, r.location, r.equipment]
  ...
  const kpis = useMemo(() => {
    ...
    const isSent = (r: RecordRow) => ["sent", "awaiting_return", "accepted", "rejected", "needs_fix", "awaiting_evidence", "in_treatment", "awaiting_validation", "closed"].includes(r.vale_status ?? "");
    const hasResult = (r: RecordRow) => !!r.vale_result;
    const approved = c((r) => ["approved", "approved_pending_exec", "approved_done"].includes(r.vale_result ?? ""));
    ...
    const approvedDays = filtered
      .filter((r) => r.sent_at && r.vale_result_at && ["approved", "approved_pending_exec", "approved_done"].includes(r.vale_result ?? ""))
      .map((r) => daysBetween(r.sent_at!, r.vale_result_at!));
    const avg = (arr: number[]) => arr.length ? (arr.reduce((a, b) => a + b, 0) / arr.length) : 0;

    return {
      total: filtered.length,
      pending_send: c((r) => ["draft", "awaiting_review", "ready"].includes(r.vale_status ?? "draft")),
      sent: c(isSent),
      pending_result: c((r) => (r.vale_status === "sent" || r.vale_status === "awaiting_return") && !hasResult(r)),
      approved, rejected,
      returned: c((r) => r.vale_result === "returned"),
      resent: c((r) => (r.vale_version ?? 1) > 1),
      closed: c((r) => r.vale_status === "closed"),
      critical: c((r) => r.priority === "alta" || r.priority === "urgente" || r.priority === "critica"),
      overdue: c((r) => !!r.vale_deadline && !r.vale_result && new Date(r.vale_deadline) < new Date()),
      no_protocol: c((r) => (r.vale_status === "sent" || r.vale_status === "awaiting_return") && !r.vale_protocol),
  ...
  const chartData = useMemo(() => {
    ...
    // 2 evolução mensal
    const byMonth = new Map<string, { name: string; enviados: number; aprovados: number; reprovados: number; reenviados: number }>();
    filtered.forEach((r) => {
      const d = new Date(r.created_at);
      const k = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      const row = byMonth.get(k) ?? { name: k, enviados: 0, aprovados: 0, reprovados: 0, reenviados: 0 };
      if (r.sent_at) row.enviados++;
      if (["approved", "approved_pending_exec", "approved_done"].includes(r.vale_result ?? "")) row.aprovados++;
    ...
    filtered.forEach((r) => {
      ...
      if (["approved", "approved_pending_exec", "approved_done"].includes(r.vale_result ?? "")) cur.a++;
      byModule.set(r.module, cur);
    });
    const approvalByModule = Array.from(byModule.entries()).map(([name, v]) => ({ name, taxa: v.total > 0 ? Math.round((v.a / v.total) * 100) : 0 }));

    // 4 motivos reprovação
    const rejReasons = g((r) => r.vale_result === "rejected" ? (r.vale_reject_category ?? "Não informado") : null);
    // 5 por área
    const byArea = g((r) => r.area).sort((a, b) => b.value - a.value).slice(0, 10);
    // 6 por criticidade
    ...
    const responseTime = Array.from(respByMonth.entries())
      .map(([name, arr]) => ({ name, dias: Math.round((arr.reduce((a, b) => a + b, 0) / arr.length) * 10) / 10 }))
      .sort((a, b) => a.name.localeCompare(b.name));

    // 8 primeira submissão vs após correção
    const firstVsCorr = [
      { name: "1ª submissão", aprovados: filtered.filter((r) => (r.vale_version ?? 1) === 1 && ["approved", "approved_pending_exec", "approved_done"].includes(r.vale_result ?? "")).length },
      { name: "Após correção", aprovados: filtered.filter((r) => (r.vale_version ?? 1) > 1 && ["approved", "approved_pending_exec", "approved_done"].includes(r.vale_result ?? "")).length },
    ];
    ...
    const deadlineBuckets = [
      { name: "No prazo", value: filtered.filter((r) => r.vale_deadline && !r.vale_result && new Date(r.vale_deadline) > now).length },
      { name: "Vence em 3 dias", value: filtered.filter((r) => r.vale_deadline && !r.vale_result && daysBetween(now.toISOString(), r.vale_deadline) <= 3 && new Date(r.vale_deadline) > now).length },
      { name: "Vencido", value: filtered.filter((r) => r.vale_deadline && !r.vale_result && new Date(r.vale_deadline) < now).length },
    ];

    return { byResult, evolution, approvalByModule, rejReasons, byArea, byPriority, responseTime, firstVsCorr, byUser, deadlineBuckets };
  }, [filtered, kpis]);

  // ── alertas
  const alerts = useMemo(() => {
    const list: { level: "danger" | "warn" | "info"; text: string; count: number }[] = [];
    const push = (level: "danger" | "warn" | "info", text: string, count: number) => count > 0 && list.push({ level, text, count });
    ...
    push("warn", "Reprovados sem tratativa registrada", filtered.filter((r) => r.vale_result === "rejected" && r.vale_status === "rejected").length);
    push("info", "Aprovados aguardando execução", filtered.filter((r) => r.vale_result === "approved_pending_exec").length);
    return list;
  }, [kpis, filtered]);
  ...
  const runInsights = useMutation({
    mutationFn: async () => {
      ...
      chartData.rejReasons.forEach((r) => { rejMap[r.name] = r.value; });
      const modMap: Record<string, number> = {};
      chartData.approvalByModule.forEach((r) => { modMap[r.name] = r.taxa; });
      const areaMap: Record<string, number> = {};
      filtered.filter((r) => !r.vale_result).forEach((r) => {
        if (!r.area) return;
  ...
  const exportCsv = useCallback(() => {
    ...
    const header = "Código,Protocolo,Módulo,Área,Título,Status,Resultado,Motivo,Criado,Enviado,Retorno,Versão";
  ...
  const exportPdf = useCallback(() => {
    ...
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text("Filtros aplicados:", 10, y); y += 5;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.text(`Período: ${filters.from || "—"} a ${filters.to || "—"}  |  Módulo: ${filters.module}  |  Status: ${filters.status}  |  Resultado: ${filters.result}  |  Área: ${filters.area || "—"}`, 10, y, { maxWidth: W - 20 }); y += 8;
    ...
    doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.text("Indicadores", 10, y); y += 5;
    ...
    kpiRows.forEach(([k, v], i) => {
      ...
      const yy = y + line * 6;
      doc.text(`${k}:`, x, yy);
      doc.setFont("helvetica", "bold"); doc.text(v, x + 55, yy);
      doc.setFont("helvetica", "normal");
    });
    y += Math.ceil(kpiRows.length / 2) * 6 + 5;

    doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.text("Motivos de reprovação", 10, y); y += 5;
    doc.setFont("helvetica", "normal"); doc.setFontSize(9);
    if (chartData.rejReasons.length === 0) { doc.text("Nenhuma reprovação no período.", 10, y); y += 6; }
    else chartData.rejReasons.forEach((r) => { doc.text(`• ${r.name}: ${r.value}`, 12, y); y += 5; });
    y += 3;

    doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.text("Tabela detalhada (primeiros 30 registros filtrados)", 10, y); y += 5;
    ...
    if (insight) {
      doc.addPage(); y = 15;
      doc.setFontSize(13); doc.setFont("helvetica", "bold"); doc.text("Análise da IA", 10, y); y += 7;
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/60 pb-4">
        <div>
          <h1 className="font-display text-2xl font-bold tracking-widest text-neon">MONITORAMENTO VALE</h1>
          <p className="text-xs text-muted-foreground">Controle de eventos enviados, resultados, aprovações, reprovações e tratativas.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => runInsights.mutate()} disabled={runInsights.isPending} className="gap-1">
            <Sparkles className="h-3 w-3" /> {runInsights.isPending ? "Analisando…" : "Insights IA"}
          </Button>
          <Button variant="outline" size="sm" onClick={exportCsv} className="gap-1"><Download className="h-3 w-3" /> CSV</Button>
          <Button variant="outline" size="sm" onClick={exportExcel} className="gap-1"><Download className="h-3 w-3" /> Excel</Button>
          <Button variant="default" size="sm" onClick={exportPdf} className="gap-1 bg-neon text-black hover:bg-neon/80"><FileText className="h-3 w-3" /> PDF</Button>
        </div>
      </div>
          ...
          {alerts.map((a) => (
            <div key={a.text}
              className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-xs ${
                a.level === "danger" ? "border-red-500/40 bg-red-500/10 text-red-300" :
                a.level === "warn" ? "border-orange-500/40 bg-orange-500/10 text-orange-300" :
      ...
      {showFilters && (
        <div className="rounded-xl border border-border/60 bg-black/30 p-3">
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
            <FiltroField label="De"><Input type="date" value={filters.from} onChange={(e) => setFilters({ ...filters, from: e.target.value })} /></FiltroField>
            <FiltroField label="Até"><Input type="date" value={filters.to} onChange={(e) => setFilters({ ...filters, to: e.target.value })} /></FiltroField>
            ...
            <FiltroField label="Resultado Vale">
              <Select value={filters.result} onValueChange={(v) => setFilters({ ...filters, result: v })}>
                ...
                <SelectContent>
                  <SelectItem value="all">Todos</SelectItem>
                  {Object.entries(RESULT_LABELS).map(([k, l]) => <SelectItem key={k} value={k}>{l}</SelectItem>)}
                </SelectContent>
              </Select>
            </FiltroField>
            <FiltroField label="Motivo reprovação">
              <Select value={filters.reject} onValueChange={(v) => setFilters({ ...filters, reject: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">Todos</SelectItem>
                  {REJECT_CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                </SelectContent>
              </Select>
            </FiltroField>
            <FiltroField label="Área"><Input value={filters.area} onChange={(e) => setFilters({ ...filters, area: e.target.value })} /></FiltroField>
            ...
            <FiltroField label="Buscar">
              <div className="relative">
                ...
                <Input className="pl-7" placeholder="código, protocolo, equipamento…" value={filters.q} onChange={(e) => setFilters({ ...filters, q: e.target.value })} />
              </div>
            </FiltroField>
            <div className="flex items-end gap-3 text-xs text-muted-foreground">
              <label className="flex items-center gap-1"><input type="checkbox" checked={filters.onlyOverdue} onChange={(e) => setFilters({ ...filters, onlyOverdue: e.target.checked })} /> Só vencidos</label>
              <label className="flex items-center gap-1"><input type="checkbox" checked={filters.onlyNoProtocol} onChange={(e) => setFilters({ ...filters, onlyNoProtocol: e.target.checked })} /> Sem protocolo</label>
            </div>
            <div className="flex items-end justify-end">
              <Button variant="ghost" size="sm" onClick={() => setFilters(emptyFilters)} className="gap-1"><X className="h-3 w-3" /> Limpar</Button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
        ...
        <KPI label="Pend. resultado" value={kpis.pending_result} icon={Clock} tone="text-blue-300" />
        ...
        <KPI label="Sem protocolo" value={kpis.no_protocol} icon={AlertTriangle} tone="text-orange-300" />
        <KPI label="Taxa aprov." value={`${kpis.approval_rate}%`} icon={TrendingUp} tone="text-emerald-300" />
        <KPI label="Taxa reprov." value={`${kpis.rejection_rate}%`} icon={TrendingUp} tone="text-red-300" />
        <KPI label="Tempo médio" value={`${kpis.avg_response}d`} icon={Timer} />
        <KPI label="Reincidências" value={kpis.recurrences} icon={HistoryIcon} tone="text-orange-300" />
      </div>
      ...
      {insight && (
        <div className="rounded-xl border border-neon/40 bg-neon/5 p-4">
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="font-display text-xs uppercase tracking-widest text-neon">Análise da IA</span>
          </div>
          <p className="whitespace-pre-wrap text-xs leading-relaxed text-foreground/90">{insight}</p>
        </div>
      )}
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={chartData.byResult} dataKey="value" nameKey="name" outerRadius={80} label>
                {chartData.byResult.map((e, i) => <Cell key={i} fill={e.color} />)}
              </Pie>
        ...
        <ChartCard title="Motivos de reprovação">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData.rejReasons}>
              <CartesianGrid stroke="#222" strokeDasharray="3 3" />
              <XAxis dataKey="name" stroke="#666" style={{ fontSize: 9 }} angle={-15} textAnchor="end" height={60} />
              <YAxis stroke="#666" style={{ fontSize: 10 }} />
              <RTooltip contentStyle={{ background: "#000", border: "1px solid #333" }} />
              <Bar dataKey="value" fill="#ef4444"
                onClick={(d: { name?: string }) => d?.name && setFilters((f) => ({ ...f, reject: d.name!, result: "rejected" }))} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Eventos por área (top 10)">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData.byArea} layout="vertical">
              ...
              <YAxis dataKey="name" type="category" stroke="#666" style={{ fontSize: 10 }} width={120} />
              <RTooltip contentStyle={{ background: "#000", border: "1px solid #333" }} />
              <Bar dataKey="value" fill={NEON}
                onClick={(d: { name?: string }) => d?.name && setFilters((f) => ({ ...f, area: d.name! }))} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Pendências por prazo">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData.deadlineBuckets}>
              ...
              <RTooltip contentStyle={{ background: "#000", border: "1px solid #333" }} />
              <Bar dataKey="value">
                {chartData.deadlineBuckets.map((_, i) => <Cell key={i} fill={["#10b981", "#eab308", "#ef4444"][i]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Registros por responsável (top 10)">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData.byUser} layout="vertical">
              <CartesianGrid stroke="#222" strokeDasharray="3 3" />
              <XAxis type="number" stroke="#666" style={{ fontSize: 10 }} />
              <YAxis dataKey="name" type="category" stroke="#666" style={{ fontSize: 9 }} width={100} tickFormatter={(v) => String(v).slice(0, 8)} />
              <RTooltip contentStyle={{ background: "#000", border: "1px solid #333" }} />
      ...
      <div className="overflow-x-auto rounded-xl border border-border/60 bg-black/20">
        <table className="w-full min-w-[1100px] text-left text-xs">
          ...
          <tbody>
            {isLoading && <tr><td colSpan={13} className="p-6 text-center text-muted-foreground">Carregando…</td></tr>}
            {!isLoading && filtered.length === 0 && <tr><td colSpan={13} className="p-6 text-center text-muted-foreground">Nenhum registro para os filtros aplicados.</td></tr>}
            {filtered.slice(0, 200).map((r) => {
              const waitDays = r.sent_at && !r.vale_result_at ? Math.round(daysBetween(r.sent_at, new Date().toISOString())) : null;
              const overdue = r.vale_deadline && !r.vale_result && new Date(r.vale_deadline) < new Date();
              return (
                <tr key={r.id} className="border-b border-border/40 hover:bg-black/30">
                  ...
                  <td className="p-2 text-[10px]">{new Date(r.created_at).toLocaleDateString("pt-BR")}</td>
                  ...
                  <td className="p-2">
                    <span className={`rounded-full border px-2 py-0.5 text-[9px] uppercase tracking-widest ${STATUS_TONE[r.vale_status ?? "draft"] ?? ""}`}>
                      {STATUS_LABELS[r.vale_status ?? "draft"]}
                    </span>
                  </td>
                  <td className="p-2 text-[10px]">{RESULT_LABELS[r.vale_result ?? ""] ?? "—"}</td>
                  <td className={`p-2 text-[10px] ${overdue ? "text-red-300 font-bold" : ""}`}>{r.vale_deadline ?? "—"}</td>
                  <td className="p-2 text-[10px]">{waitDays !== null ? `${waitDays}d` : "—"}</td>
                  <td className="p-2 text-[10px] text-center">
                    {(r.vale_version ?? 1) > 1
                      ? <button onClick={() => setVersionsTarget(r)} className="text-neon underline">v{r.vale_version}</button>
                      : "v1"}
                  </td>
                  <td className="p-2">
                    <div className="flex justify-end gap-1">
                      <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px]" onClick={() => setDetailTarget(r)}>Abrir</Button>
                      {!r.sent_at && (
                        <Button size="sm" variant="outline" className="h-6 px-2 text-[10px]" onClick={() => setSendTarget(r)}>
                          <Send className="mr-1 h-3 w-3" /> Enviar
                        </Button>
                      )}
                      {r.sent_at && !r.vale_result && (
                        <Button size="sm" variant="outline" className="h-6 px-2 text-[10px]" onClick={() => setResultTarget(r)}>Resultado</Button>
                      )}
                      {r.vale_result === "rejected" || r.vale_result === "returned" || r.vale_result === "awaiting_complement" ? (
                        <Button size="sm" variant="outline" className="h-6 px-2 text-[10px]" onClick={() => setRevisionTarget(r)}>
                          <RotateCcw className="mr-1 h-3 w-3" /> Corrigir
                        </Button>
                      ) : null}
                      <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px]" onClick={() => setHistoryTarget(r)}>
      ...
      {sendTarget && (
        <SendDialog record={sendTarget} onClose={() => setSendTarget(null)} onSaved={() => { setSendTarget(null); invalidate(); }} />
      )}
      {resultTarget && (
        <ResultDialog record={resultTarget} onClose={() => setResultTarget(null)} onSaved={() => { setResultTarget(null); invalidate(); }} />
      )}
      {revisionTarget && (
        <RevisionDialog record={revisionTarget} onClose={() => setRevisionTarget(null)} onSaved={() => { setRevisionTarget(null); invalidate(); }} />
      )}
      {versionsTarget && (
      ...
      {detailTarget && (
        <DetailDrawer record={detailTarget} onClose={() => setDetailTarget(null)}
          onSend={() => { setSendTarget(detailTarget); setDetailTarget(null); }}
          onResult={() => { setResultTarget(detailTarget); setDetailTarget(null); }}
          onHistory={() => { setHistoryTarget(detailTarget); setDetailTarget(null); }}
          onVersions={() => { setVersionsTarget(detailTarget); setDetailTarget(null); }}
        />
      )}
      {historyTarget && (
...
function KPI({ label, value, tone, icon: Icon }: { label: string; value: number | string; tone?: string; icon: React.ComponentType<{ className?: string }> }) {
  return (
    <div className="rounded-xl border border-border/60 bg-black/30 p-2.5">
      ...
      <div className={`mt-1 font-display text-xl font-bold ${tone ?? "text-foreground"}`}>{value}</div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-black/30 p-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <h3 className="font-display text-[11px] uppercase tracking-widest text-foreground/90">{title}</h3>
      </div>
      {children}
    </div>
  );
...
// ─────────────────────────── SendDialog (mantido)
function SendDialog({ record, onClose, onSaved }: { record: RecordRow; onClose: () => void; onSaved: () => void }) {
  ...
  const send = useMutation({
    mutationFn: async () => {
      ...
      if (proof) {
        ...
        const up = await supabase.storage.from("inspections").upload(path, proof, { contentType: proof.type });
        if (up.error) throw up.error;
        const { data: s } = await supabase.storage.from("inspections").createSignedUrl(path, 60 * 60 * 24 * 365);
    ...
    onSuccess: () => { toast.success("Envio confirmado. Registro pendente de resultado Vale."); onSaved(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[92dvh] max-w-lg overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display uppercase tracking-widest text-neon">Confirmar envio para a Vale</DialogTitle>
          <DialogDescription>
            Código interno <span className="font-mono text-neon">{record.internal_code ?? "—"}</span>
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <div className="grid gap-1">
            <Label>Protocolo Vale *</Label>
            <Input value={protocol} onChange={(e) => setProtocol(e.target.value)} placeholder="ex.: VALE-N3-987654" />
          </div>
          <div className="grid gap-1">
          ...
          {override && (
            <Textarea rows={2} placeholder="Justificativa obrigatória" value={justification} onChange={(e) => setJustification(e.target.value)} />
          )}
...
// ─────────────────────────── ResultDialog
function ResultDialog({ record, onClose, onSaved }: { record: RecordRow; onClose: () => void; onSaved: () => void }) {
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      const finalResult = result === "approved"
        ? (approvalKind === "done" ? "approved_done" : "approved_pending_exec")
        : result;
      await register({
        data: {
          record_id: record.id,
    ...
    onSuccess: () => { toast.success("Resultado Vale registrado."); onSaved(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[92dvh] max-w-xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display uppercase tracking-widest text-neon">Registrar resultado Vale</DialogTitle>
          <DialogDescription>
            Código <span className="font-mono text-neon">{record.internal_code ?? "—"}</span> · Protocolo{" "}
            <span className="font-mono text-neon">{record.vale_protocol ?? "—"}</span>
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <div className="grid gap-1">
          ...
          {result === "approved" && (
            <div className="grid gap-1">
              ...
              <Select value={approvalKind} onValueChange={(v) => setApprovalKind(v as "pending_exec" | "done")}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              <p className="text-[10px] text-muted-foreground">Não encerramos automaticamente na aprovação. Anexe evidência real para concluir.</p>
            </div>
          )}
            ...
            <>
              <div className="grid gap-1">
                ...
                <Select value={rejectCat} onValueChange={setRejectCat}>
                  ...
                  <SelectContent>{REJECT_CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="grid gap-1">
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={onClose}>Cancelar</Button>
            <Button className="bg-neon text-black hover:bg-neon/80" disabled={!canSave} onClick={() => mut.mutate()}>
...
// ─────────────────────────── RevisionDialog
function RevisionDialog({ record, onClose, onSaved }: { record: RecordRow; onClose: () => void; onSaved: () => void }) {
  const revise = useServerFn(createValeRevision);
  const [note, setNote] = useState("");
  const mut = useMutation({
    mutationFn: () => revise({ data: { record_id: record.id, changes_note: note.trim() } }),
    onSuccess: (d: { version: number }) => { toast.success(`Nova versão v${d.version} criada. Corrija e reenvie.`); onSaved(); },
    onError: (e: Error) => toast.error(e.message),
  });
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="font-display uppercase tracking-widest text-neon">Corrigir e criar nova versão</DialogTitle>
          <DialogDescription>
            A versão anterior será preservada no histórico. Este registro voltará para "Pendente de envio".
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-1">
          <Label>Alterações realizadas *</Label>
          <Textarea rows={4} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Descreva o que foi corrigido…" />
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancelar</Button>
          <Button className="bg-neon text-black hover:bg-neon/80" disabled={note.trim().length < 3 || mut.isPending} onClick={() => mut.mutate()}>
...
function VersionsDialog({ record, onClose }: { record: RecordRow; onClose: () => void }) {
  ...
  type V = {
    id: string; version: number; sent_at: string | null; channel: string | null;
    protocol: string | null; result: string | null; result_at: string | null;
    result_note: string | null; reject_category: string | null;
    reject_reason: string | null; changes_note: string | null;
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display uppercase tracking-widest text-neon">Versões do registro</DialogTitle>
          <DialogDescription>{record.internal_code ?? record.id}</DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          {versions.length === 0 && <p className="text-xs text-muted-foreground">Sem versões anteriores. Este é o primeiro envio.</p>}
          {versions.map((v) => (
            <div key={v.id} className="rounded-lg border border-border/60 bg-black/40 p-3">
              <div className="mb-1 flex items-center justify-between">
                <span className="font-display text-xs uppercase tracking-widest text-neon">Versão {v.version}</span>
                <span className="text-[10px] text-muted-foreground">
                  {v.sent_at ? new Date(v.sent_at).toLocaleString("pt-BR") : "não enviada"} · {v.channel ?? "—"}
                </span>
              </div>
              <div className="grid gap-2 md:grid-cols-[120px_1fr]">
                {v.snapshot?.photo_url && <img src={v.snapshot.photo_url} alt="" className="h-24 w-24 rounded border border-border object-cover" />}
                <div className="space-y-1 text-[11px]">
                  <p><span className="text-muted-foreground">Protocolo:</span> <span className="font-mono">{v.protocol ?? "—"}</span></p>
                  <p><span className="text-muted-foreground">Resultado:</span> {RESULT_LABELS[v.result ?? ""] ?? "—"}</p>
                  {v.reject_category && <p><span className="text-muted-foreground">Motivo:</span> {v.reject_category} — {v.reject_reason}</p>}
                  {v.result_note && <p><span className="text-muted-foreground">Parecer:</span> {v.result_note}</p>}
                  {v.changes_note && <p><span className="text-muted-foreground">Alterações desta versão:</span> {v.changes_note}</p>}
                </div>
              </div>
            </div>
          ))}
          <div className="rounded-lg border border-neon/40 bg-neon/5 p-3">
            <div className="mb-1 flex items-center justify-between">
              <span className="font-display text-xs uppercase tracking-widest text-neon">Versão atual v{record.vale_version ?? 1}</span>
              <span className="text-[10px] text-muted-foreground">{STATUS_LABELS[record.vale_status ?? "draft"]}</span>
            </div>
            <p className="text-[11px]"><span className="text-muted-foreground">Resultado:</span> {RESULT_LABELS[record.vale_result ?? ""] ?? "aguardando"}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
...
function DetailDrawer({ record, onClose, onSend, onResult, onHistory, onVersions }: {
  record: RecordRow; onClose: () => void;
  onSend: () => void; onResult: () => void; onHistory: () => void; onVersions: () => void;
}) {
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display uppercase tracking-widest text-neon">Detalhamento do evento</DialogTitle>
          <DialogDescription>
            <span className="font-mono text-neon">{record.internal_code}</span> · v{record.vale_version ?? 1}
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-3 md:grid-cols-[160px_1fr]">
          {record.photo_url ? (
            <img src={record.photo_url} alt="" className="h-40 w-40 rounded-lg border border-border object-cover" />
          ) : (
          ...
          <div className="space-y-1 text-xs">
            ...
            <Field label="Área / Local / Equipamento">{[record.area, record.location, record.equipment].filter(Boolean).join(" · ")}</Field>
        ...
        <div className="grid gap-2 md:grid-cols-2 text-xs">
          <Field label="Status">
            <span className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-widest ${STATUS_TONE[record.vale_status ?? "draft"] ?? ""}`}>
          ...
          <Field label="Data envio">{record.sent_at ? new Date(record.sent_at).toLocaleString("pt-BR") : "—"}</Field>
          <Field label="Canal">{record.sent_channel ?? "—"}</Field>
          <Field label="Data retorno">{record.vale_result_at ? new Date(record.vale_result_at).toLocaleString("pt-BR") : "—"}</Field>
        ...
        {record.vale_reject_category && (
          <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-2 text-xs">
            <p className="font-bold text-red-300">Motivo da reprovação: {record.vale_reject_category}</p>
        ...
        <DialogFooter className="flex-wrap gap-2">
          {!record.sent_at && <Button size="sm" onClick={onSend} className="bg-neon text-black hover:bg-neon/80">Confirmar envio</Button>}
          {record.sent_at && !record.vale_result && <Button size="sm" onClick={onResult}>Registrar resultado</Button>}
          <Button size="sm" variant="outline" onClick={onVersions}>Ver versões</Button>
          <Button size="sm" variant="outline" onClick={onHistory}>Histórico</Button>
...
function HistoryDialog({ record, onClose }: { record: RecordRow; onClose: () => void }) {
  ...
  type Entry = { id: string; action: string; from_status: string | null; to_status: string | null; justification: string | null; proof_url: string | null; created_at: string };
  const entries = (data?.history ?? []) as Entry[];
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[92dvh] max-w-lg overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display uppercase tracking-widest text-neon">Histórico do registro</DialogTitle>
        ...
        <div className="space-y-2">
          ...
          {entries.map((e) => (
            <div key={e.id} className="rounded-lg border border-border/60 bg-black/30 p-2 text-xs">
              <div className="flex items-center justify-between">
                <span className="font-display text-[10px] uppercase tracking-widest text-neon">{e.action}</span>
                <span className="text-[10px] text-muted-foreground">{new Date(e.created_at).toLocaleString("pt-BR")}</span>
              </div>
              {(e.from_status || e.to_status) && (
                <p className="mt-1 text-[11px] text-muted-foreground">
                  {e.from_status ?? "—"} → <span className="text-foreground">{e.to_status ?? "—"}</span>
                </p>
              )}
              {e.justification && <p className="mt-1 text-[11px]">{e.justification}</p>}
              {e.proof_url && (
                <a href={e.proof_url} target="_blank" rel="noreferrer" className="mt-1 inline-block text-[11px] text-neon underline">
                  Ver comprovante
                </a>
              )}
            </div>
