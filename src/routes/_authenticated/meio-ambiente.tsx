...
import { Leaf, Plus, AlertTriangle, CheckCircle2, Clock, ShieldCheck, Search, FileText, Sparkles } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/meio-ambiente")({
  head: () => ({ meta: [{ title: "Meio Ambiente · VALETECH" }] }),
...
const levelLabel: Record<string, string> = {
  muito_baixo: "Muito baixo", baixo: "Baixo", moderado: "Moderado", alto: "Alto", critico: "Crítico",
};
...
function MeioAmbientePage() {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("records")
        .select("id, internal_code, title, description, area, location, equipment, status, priority, photo_url, created_at, env_categories, env_score_before, env_level_before, env_score_after, env_level_after, env_effectiveness, env_audit_verdict, env_after_photo_url")
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...
      criticos: records.filter((r) => r.env_level_before === "critico" || r.env_level_before === "alto").length,
      emergencias: records.filter((r) => r.env_categories?.includes("emergencia_ambiental")).length,
      aguardEvid: records.filter((r) => r.status === "em_andamento" && !r.env_after_photo_url).length,
  ...
  const filtered = useMemo(() => {
    ...
    if (tab === "criticos") list = list.filter((r) => r.env_level_before === "critico" || r.env_level_before === "alto");
    else if (tab === "vencidas") {
      const now = Date.now();
      list = list.filter((r) => r.status !== "concluido" && now - new Date(r.created_at).getTime() > 15 * 24 * 3600 * 1000 && !r.env_after_photo_url);
    }
    else if (tab === "aguard_evid") list = list.filter((r) => r.status === "em_andamento" && !r.env_after_photo_url);
    else if (tab === "aguard_valid") list = list.filter((r) => r.env_after_photo_url && !r.env_audit_verdict);
    else if (tab === "encerrados") list = list.filter((r) => r.status === "concluido");
    return list;
  }, [records, search, tab]);

  return (
    <ModuleShell icon={Leaf} title="Meio Ambiente" subtitle="Inspeção, auditoria, análise de impactos e acompanhamento de ações ambientais com apoio da IA.">
      <div className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2">
          {[
            ...
            <div key={k.k} className="rounded-lg border border-border bg-card/50 p-3">
              <div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-muted-foreground"><k.i className={`w-3 h-3 ${k.c}`} />{k.k}</div>
        ...
        <div className="flex gap-2 flex-wrap">
          <Button onClick={() => setOpen(true)} className="bg-neon text-black hover:bg-neon/90">
            <Plus className="w-4 h-4 mr-1" />Registrar condição ambiental
          </Button>
          <Button variant="outline" onClick={() => setOpen(true)}>
            <Sparkles className="w-4 h-4 mr-1" />Analisar com a IA
          </Button>
          <div className="relative flex-1 min-w-[200px]">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <Input placeholder="Buscar por título, área, equipamento, código..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
          </div>
        </div>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value={tab} className="mt-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {filtered.map((r) => (
                <div key={r.id} className="rounded-lg border border-border bg-card/50 overflow-hidden hover:border-neon/40 transition">
                  {r.photo_url && (
                  ...
                  <div className="p-3 space-y-1.5">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {r.internal_code && <Badge variant="outline" className="text-[10px]">{r.internal_code}</Badge>}
                      {r.env_level_before && (
                      ...
                      {r.env_after_photo_url && !r.env_audit_verdict && (
                        <Badge className="text-[10px] bg-cyan-500/15 text-cyan-300">Aguardando validação</Badge>
                      )}
                      {r.env_audit_verdict === "conforme" && (
                        <Badge className="text-[10px] bg-neon/15 text-neon">Validado</Badge>
                      )}
                    </div>
                    <div className="font-medium text-sm line-clamp-1">{r.title ?? "Sem título"}</div>
                    ...
                    {r.env_categories && r.env_categories.length > 0 && (
                      <div className="flex flex-wrap gap-1 pt-1">
                        {r.env_categories.slice(0, 3).map((c) => (
                          <span key={c} className="text-[9px] px-1.5 py-0.5 rounded bg-muted/40 text-muted-foreground">{c.replace(/_/g, " ")}</span>
                        ))}
                      </div>
                    )}
                  </div>
