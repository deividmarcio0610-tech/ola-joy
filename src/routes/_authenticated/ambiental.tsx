...
import { Leaf, Send, Loader2, Upload, X, FileText, Image as ImageIcon, Sheet, Video, Mic, Sparkles, ClipboardList, Download, Lightbulb } from "lucide-react";
...
import { classifyAttachment, MAX_ATTACHMENT_MB, type EnvAttachmentKind, type EnvAnalysisResult } from "@/lib/environmental/schema";
import { analisarAmbientalN3, conversarAmbiental, fileToDataUrl, uploadEnvAttachment, type EnvChatTurn } from "@/lib/environmental/analyze";
...
export const Route = createFileRoute("/_authenticated/ambiental")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Auditoria ambiental N3 automática com conselho multidisciplinar de especialistas, plano de ação 5W2H e histórico." },
  ...
  return (
    ...
    >
      <Tabs defaultValue="auditoria" className="w-full">
        ...
        <TabsContent value="chat" className="mt-6"><EnvChatPanel /></TabsContent>
        <TabsContent value="auditoria" className="mt-6"><EnvAuditPanel /></TabsContent>
        <TabsContent value="historico" className="mt-6"><EnvHistoryPanel /></TabsContent>
      </Tabs>
    </ModuleShell>
  );
}
...
function EnvChatPanel() {
  ...
  async function send() {
    ...
    try {
      ...
      const userTurn: EnvChatTurn = { role: "user", content: text || "(anexos enviados)", images: imgs };
  ...
  return (
    <Card>
      <CardHeader><CardTitle className="text-base">Chat Ambiental Contínuo</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <div ref={listRef} className="max-h-[420px] min-h-[220px] overflow-y-auto rounded-lg border border-border bg-background/40 p-3 space-y-3">
          {messages.length === 0 && (
            <div className="text-sm text-muted-foreground">
              Pergunte sobre ISO 14001, PNRS, CONAMA, licenciamento, resíduos, efluentes ou emergências. Anexe imagens para análise pontual.
            </div>
          )}
          {messages.map((m, i) => (
            <div key={i} className={m.role === "user" ? "text-right" : ""}>
              <div className={`inline-block max-w-[92%] rounded-lg px-3 py-2 text-sm ${m.role === "user" ? "bg-primary text-primary-foreground" : "bg-card"}`}>
                <ReactMarkdown>{m.content}</ReactMarkdown>
                {m.images && m.images.length > 0 && (
                  <div className="mt-2 flex gap-1">
                    {m.images.map((src, j) => <img key={j} src={src} alt="anexo" className="h-16 w-16 rounded object-cover" />)}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            {pending.map((f, i) => (
              <Badge key={i} variant="outline" className="gap-1">
                {f.name}
                <button onClick={() => setPending(pending.filter((_, k) => k !== i))} className="ml-1"><X className="h-3 w-3" /></button>
              </Badge>
            ))}
          </div>
        )}
        <div className="flex items-end gap-2">
          <label className="cursor-pointer rounded-md border border-border p-2 hover:bg-accent">
            ...
            <input type="file" accept="image/*" multiple className="hidden" onChange={(e) => {
              const files = Array.from(e.target.files ?? []).filter((f) => f.size <= MAX_ATTACHMENT_MB * 1024 * 1024);
          ...
          <Button onClick={send} disabled={loading}>{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</Button>
        </div>
      </CardContent>
    </Card>
  );
...
const KIND_ICON: Record<EnvAttachmentKind, typeof ImageIcon> = {
  image: ImageIcon, pdf: FileText, spreadsheet: Sheet, video: Video, audio: Mic, other: FileText,
};
...
function EnvAuditPanel() {
  ...
  async function analyze() {
    ...
    try {
      ...
      const result = await analisarAmbientalN3({
        description, area, location, images: imgs,
  ...
  async function save() {
    ...
    try {
      ...
      const uploaded: Array<{ kind: EnvAttachmentKind; path: string; signedUrl: string | null; filename: string; mime: string | null; size: number | null }> = [];
      for (const a of attachments) {
        try {
          const up = await uploadEnvAttachment(tmpAuditId, a.file);
          uploaded.push({ kind: a.kind, path: up.path, signedUrl: up.signedUrl, filename: a.file.name, mime: a.file.type || null, size: a.file.size });
        } catch (err) {
      ...
      setTitle(""); setArea(""); setLocation(""); setDescription(""); setAttachments([]); setAnalysis(null);
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Input placeholder="Local / GPS" value={location} onChange={(e) => setLocation(e.target.value)} />
          </div>
          <Textarea rows={4} placeholder="Descreva a condição, o contexto e o que precisa ser auditado."
          ...
          <div>
            <label className="flex cursor-pointer items-center gap-2 rounded-md border border-dashed border-border p-3 text-sm hover:bg-accent">
              <Upload className="h-4 w-4" />
              <span>Anexar fotos, PDFs, planilhas, vídeos ou áudios (até {MAX_ATTACHMENT_MB}MB cada)</span>
            ...
            {attachments.length > 0 && (
              <div className="mt-2 grid grid-cols-2 gap-2">
                {attachments.map((a, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2 rounded border border-border bg-card/60 p-2 text-xs">
                      <Ic className="h-4 w-4 text-neon" />
                      <span className="truncate flex-1">{a.file.name}</span>
                      <button onClick={() => setAttachments(attachments.filter((_, k) => k !== i))}><X className="h-3 w-3" /></button>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="flex gap-2 pt-1">
            <Button onClick={analyze} disabled={analyzing} className="flex-1">
              {analyzing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Analisando…</> : <><Sparkles className="mr-2 h-4 w-4" /> Analisar com Conselho N3</>}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {!analysis && <p className="text-sm text-muted-foreground">Envie evidências e clique em analisar. O conselho multidisciplinar preencherá findings N3, matriz de risco, requisitos legais, cenários (econômica/recomendada/ideal), PDCA e ações 5W2H.</p>}
    ...
    <div className="space-y-4 text-sm">
      ...
      {a.necessita_mais_evidencia && (
        <div className="rounded border border-yellow-500/40 bg-yellow-500/10 p-2 text-xs text-yellow-200">
          Evidência insuficiente: {a.motivo_evidencia_insuficiente || "necessária validação em campo."}
        </div>
      )}
      ...
      {a.categorias?.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {a.categorias.map((c) => <Badge key={c} variant="secondary">{c}</Badge>)}
        </div>
      )}
      <section>
        <h4 className="mb-1 font-semibold">Matriz</h4>
        <div className="grid grid-cols-3 gap-2 text-xs">
          {Object.entries(a.matriz ?? {}).filter(([k]) => k !== "justificativa").map(([k, v]) => (
        ...
        {a.matriz?.justificativa && <p className="mt-2 text-xs text-muted-foreground">{a.matriz.justificativa}</p>}
      </section>
      {a.requisitos_legais?.length > 0 && (
        <section>
          <h4 className="mb-1 font-semibold">Requisitos legais</h4>
          <ul className="list-disc space-y-1 pl-4">
            {a.requisitos_legais.map((r, i) => <li key={i}><strong>{r.norma}</strong>{r.artigo ? ` · ${r.artigo}` : ""} — {r.descricao}</li>)}
          </ul>
        </section>
      )}
      ...
      {a.parecer_auditoria && (
        <section className="rounded border border-border bg-card/40 p-3">
          ...
          <p><strong>Observado:</strong> {a.parecer_auditoria.observado}</p>
          <p><strong>Critério:</strong> {a.parecer_auditoria.criterio}</p>
          <p><strong>Achado:</strong> {a.parecer_auditoria.tipo_achado} · prioridade {a.parecer_auditoria.prioridade}</p>
          <p><strong>Consequência:</strong> {a.parecer_auditoria.consequencia}</p>
          <p><strong>Recomendação:</strong> {a.parecer_auditoria.recomendacao}</p>
        </section>
      )}
      {a.cenarios && (
        <section>
      ...
      {a.pdca && (
        <section className="rounded border border-border bg-card/40 p-3">
          ...
          <p><strong>P — Planejar:</strong> {a.pdca.planejar}</p>
          <p><strong>D — Executar:</strong> {a.pdca.executar}</p>
          <p><strong>C — Verificar:</strong> {a.pdca.verificar}</p>
          <p><strong>A — Agir:</strong> {a.pdca.agir}</p>
        </section>
      )}
...
function ScenarioCard({ label, s, tone }: { label: string; tone: "muted" | "accent" | "primary"; s: NonNullable<EnvAnalysisResult["cenarios"]>["economica"] }) {
  const toneClass = tone === "accent" ? "border-emerald-500/60 bg-emerald-500/5" : tone === "primary" ? "border-sky-500/60 bg-sky-500/5" : "border-border bg-card/40";
  return (
    <div className={`rounded border ${toneClass} p-2 text-xs`}>
      <div className="mb-1 flex items-center justify-between">
        <span className="font-semibold">{label}</span>
        <Badge variant="outline" className="text-[10px]">{s?.prazo ?? "—"}</Badge>
      </div>
      <p className="font-medium">{s?.titulo}</p>
      <p className="mt-1 text-muted-foreground">{s?.descricao}</p>
      <div className="mt-2 space-y-0.5 text-muted-foreground">
        <div><strong>Custo:</strong> {s?.custo_estimado}</div>
        <div><strong>Eficiência:</strong> {s?.eficiencia}</div>
        <div><strong>Risco residual:</strong> {s?.risco_residual}</div>
        <div><strong>Vida útil:</strong> {s?.vida_util}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded border border-emerald-500/40 bg-emerald-500/5 p-3">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Título, problema, aspecto, impacto, solução recomendada, meta e indicador são preenchidos automaticamente no módulo Kaisen.
      </p>
    </div>
  );
}

function ActionBlock({ title, items }: { title: string; items?: Array<{ descricao: string; o_que?: string; quem?: string; quando?: string; onde?: string; como?: string; prioridade?: string; controle?: string }> }) {
        ...
        {items.map((a, i) => (
          <li key={i} className="rounded border border-border bg-card/40 p-2 text-xs">
            ...
            <div className="mt-1 grid grid-cols-2 gap-1 text-muted-foreground">
              {a.o_que && <span><strong>O quê:</strong> {a.o_que}</span>}
              {a.quem && <span><strong>Quem:</strong> {a.quem}</span>}
              {a.onde && <span><strong>Onde:</strong> {a.onde}</span>}
              {a.quando && <span><strong>Quando:</strong> {a.quando}</span>}
              {a.como && <span className="col-span-2"><strong>Como:</strong> {a.como}</span>}
              {a.prioridade && <Badge variant="outline" className="mt-1">prioridade: {a.prioridade}</Badge>}
              {a.controle && <Badge variant="outline" className="mt-1">controle: {a.controle}</Badge>}
            </div>
          </li>
        ))}
      </ul>
...
function EnvHistoryPanel() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        <CardHeader><CardTitle className="text-base">Auditorias recentes</CardTitle></CardHeader>
        <CardContent>
          {isLoading && <p className="text-sm text-muted-foreground">Carregando…</p>}
          {!isLoading && audits.length === 0 && <p className="text-sm text-muted-foreground">Nenhuma auditoria salva.</p>}
          <ul className="space-y-2">
            {audits.map((a) => (
              <li key={a.id}>
                <button
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground">
                    {a.area || "—"} · criticidade {a.criticality || "—"} · {new Date(a.created_at).toLocaleString("pt-BR")}
                  </div>
                </button>
              </li>
            ))}
            ...
            return (
              <div className="space-y-3 text-sm">
                ...
                <div className="text-xs">
                  Findings: {detail.data.findings.length} · Ações: {detail.data.actions.length} · Anexos: {detail.data.attachments.length}
                </div>
...
function mapCriticality(sev?: string): "controlada"|"baixa"|"moderada"|"alta"|"muito_alta"|"critica" | null {
  switch (sev) {
    case "muito_baixo": return "controlada";
...
function mapControl(c?: string): "eliminacao"|"substituicao"|"reducao_geracao"|"engenharia"|"contencao"|"monitoramento"|"administrativo"|"treinamento"|"emergencia"|"compensacao" | null {
...
function mapFindingClassification(t?: string) {
  switch (t) {
    case "conforme": return "boa_pratica";
    case "conforme_com_observacao": return "observacao";
    case "oportunidade_melhoria": return "oportunidade";
    case "nc_menor": return "nc_documental";
    case "nc_maior": return "nc_operacional";
    case "critica": return "risco_critico";
    case "emergencia_ambiental": return "emergencia_potencial";
    default: return null;
  }
}
function actionRow(a: { descricao: string; o_que?: string; por_que?: string; onde?: string; quando?: string; quem?: string; como?: string; quanto?: string; controle?: string; prioridade?: string; evidencia_requerida?: string }, tier: "imediata"|"kaisen"|"engenharia"|"inovacao") {
  return {
    tier,
    ...
    priority: (["baixa","media","alta","critica"].includes(a.prioridade ?? "") ? a.prioridade : "media") as "baixa"|"media"|"alta"|"critica",
