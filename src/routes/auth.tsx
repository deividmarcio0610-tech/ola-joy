...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [{ title: "Entrar · VisionGuard AI" }],
  }),
  component: AuthPage,
});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.session) {
          toast.success("Conta criada. Bem-vindo ao VisionGuard AI.");
    ...
    <div className="mx-auto flex min-h-[80vh] max-w-md items-center justify-center">
      <div className="w-full rounded-2xl border border-border bg-card/60 p-8 backdrop-blur">
        <div className="mb-6 text-center">
          ...
          />
          <div className="inline-flex items-center gap-2 rounded-full border border-neon/30 bg-neon/5 px-3 py-1 font-display text-[10px] uppercase tracking-[0.3em] text-neon">
            VisionGuard AI
          </div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {mode === "forgot"
              ...
              : "Acesse a plataforma corporativa VisionGuard AI."}
          </p>
        </div>
