...
export function computeLightningRisk(input: {
  ...
  if (cape !== null) {
    if (cape >= 2500) { score += 40; reasons.push(`CAPE muito alta (${cape.toFixed(0)} J/kg).`); }
    else if (cape >= 1500) { score += 28; reasons.push(`CAPE alta (${cape.toFixed(0)} J/kg).`); }
    else if (cape >= 800) { score += 16; reasons.push(`CAPE moderada (${cape.toFixed(0)} J/kg).`); }
    else if (cape >= 300) { score += 6; reasons.push(`CAPE fraca (${cape.toFixed(0)} J/kg).`); }
  }
  if (li !== null) {
    if (li <= -6) { score += 20; reasons.push(`Lifted Index ${li.toFixed(1)} (instabilidade extrema).`); }
    else if (li <= -3) { score += 12; reasons.push(`Lifted Index ${li.toFixed(1)} (instabilidade forte).`); }
    else if (li <= 0) { score += 5; reasons.push(`Lifted Index ${li.toFixed(1)} (instabilidade leve).`); }
  }
  if (probNext3h >= 80) { score += 10; reasons.push(`Prob. de chuva ${probNext3h}% nas próximas 3h.`); }
  else if (probNext3h >= 60) { score += 5; }
