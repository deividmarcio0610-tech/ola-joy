...
const ASSET_PRICE_RANGES: Array<{
  pattern: RegExp;
  min: number;
  max: number;
  label: string;
}> = [
  { pattern: /^WIN/i, min: 10_000, max: 500_000, label: "WINFUT" },
  { pattern: /^WDO/i, min: 1_000, max: 20_000, label: "WDOFUT" },
];
...
export function assetScaleIssue(asset: string, anchors: ScaleAnchor[]): string | null {
  const expected = ASSET_PRICE_RANGES.find((range) => range.pattern.test(asset.trim()));
