...
function h2(text: string): Content { return { text, style: "h2", margin: [0, 12, 0, 6] }; }
function p(text: string): Content { return { text, style: "body" }; }
function bullet(items: string[]): Content { return { ul: items.length ? items : ["—"], style: "body" }; }
...
export async function generateReportPdf(report: TechnicalReport): Promise<Blob> {
  ...
  const cover: Content = {
    stack: [
      ...
      { text: report.identification.tipo_analise, style: "coverSub", absolutePosition: { x: 60, y: 130 } },
      { text: "\n\n\n" },
      table(["Campo", "Valor"], [
        ...
        ["Responsável pela validação", report.identification.responsavel_validacao ?? "A definir"],
      ...
      qr ? { text: "Consulta rápida via QR Code", style: "caption", alignment: "right" } : { text: "" },
    ],
  };
  ...
  const execSection: Content[] = [
    ...
    h2("Principais oportunidades"), bullet(summary.principais_oportunidades),
    h2("Riscos prioritários"), bullet(summary.riscos_prioritarios),
    h2("Melhorias sugeridas"), bullet(summary.melhorias_sugeridas),
    h2("Economia potencial"), p(summary.economia_potencial),
    h2("Prioridade geral / Confiança da IA"),
    table(["Prioridade", "Confiança IA"], [[summary.prioridade_geral.toUpperCase(), `${summary.nivel_confianca_ia}%`]]),
    h2("Limitações da análise"), bullet(summary.limitacoes),
  ];
  ...
  const imagesSection: Content[] = [
    ...
    original ? { image: original, fit: [480, 320], alignment: "center" } : p("(imagem original não disponível)"),
    p("Nota: a imagem original nunca é alterada."),
    h2("3.2 Imagem com marcações"),
    marked ? { image: marked, fit: [480, 320], alignment: "center" } : p("(imagem marcada não disponível)"),
    report.markings.length
      ? table(["#","Marcação","Especialidade","Severidade","Ref."],
          report.markings.map((m) => [String(m.id), m.label, m.specialty ? SPECIALTY_LABEL[m.specialty] : "—", m.severity ?? "—", m.ref_section ?? "—"]))
  ...
  const specialtiesSection: Content[] = [
    ...
    ...report.specialties.flatMap((s): Content[] => [
      ...
      { text: "Observações", style: "label" }, p(s.observacoes),
      { text: "Evidências", style: "label" }, bullet(s.evidencias),
      { text: "Limitações", style: "label" }, bullet(s.limitacoes),
      { text: "Riscos", style: "label" }, bullet(s.riscos),
      { text: "Oportunidades", style: "label" }, bullet(s.oportunidades),
      { text: "Recomendações", style: "label" }, bullet(s.recomendacoes),
    ]),
  ];
  ...
  const innovationSection: Content[] = [
    ...
    ...report.innovative_ideas.flatMap((i): Content[] => [
      h2(i.conceito),
      p(i.funcionamento),
      table(["Dificuldade","Replicação","Investimento","Retorno"], [[i.dificuldade, i.replicacao, BUDGET_TIER_LABEL[i.investimento], i.retorno_esperado]]),
      { text: "Benefícios", style: "label" }, bullet(i.beneficios),
      { text: "Riscos", style: "label" }, bullet(i.riscos),
    ]),
  ];

  const altSection: Content[] = [
    h1("6. Comparação de Alternativas", "sec6"),
    table(["Critério","Econômica","Recomendada","Ideal"],
  ...
  const matrixSection: Content[] = [
    ...
    table(["Item","Impacto","Urgência","Esforço","Custo","Benefício","ROI","Segurança","Replicação"],
      report.priority_matrix.map((r) => [r.item, String(r.impacto), String(r.urgencia), String(r.esforco), String(r.custo), String(r.beneficio), String(r.roi), String(r.seguranca), String(r.replicacao)])),
  ];
  ...
  const planSection: Content[] = [
    ...
    table(["O quê","Por quê","Onde","Quando","Quem","Como","Quanto","Prioridade"],
      report.action_plan.map((a) => [a.what, a.why, a.where, a.when, a.who, a.how, BUDGET_TIER_LABEL[a.how_much], a.priority])),
  ];

  const pdcaSection: Content[] = [
    h1("10. PDCA", "sec10"),
    h2("Planejar (P)"), bullet(report.pdca.plan),
    h2("Executar (D)"), bullet(report.pdca.do),
    h2("Verificar (C)"), bullet(report.pdca.check),
    h2("Agir (A)"), bullet(report.pdca.act),
  ];

  const risksSection: Content[] = [
    h1("11. Análise de Riscos", "sec11"),
    table(["Risco","Consequência","Prob.","Sev.","Crit.","Controles atuais","Controles sugeridos"],
      report.risks.map((r) => [r.risco, r.consequencia, String(r.probabilidade), String(r.severidade), String(r.criticidade), r.controles_existentes, r.controles_sugeridos])),
  ];

  const b = report.benefits;
  const benefitsSection: Content[] = [
    h1("12. Benefícios", "sec12"),
    h2("Segurança"), bullet(b.seguranca),
    h2("Financeiros"), bullet(b.financeiros),
    h2("Operacionais"), bullet(b.operacionais),
    h2("Ambientais"), bullet(b.ambientais),
    h2("Ergonômicos"), bullet(b.ergonomicos),
    h2("Manutenção"), bullet(b.manutencao),
  ];

  const scheduleSection: Content[] = [
    h1("13. Cronograma Preliminar", "sec13"),
    table(["Fase","Início","Duração","Responsável"],
  ...
  const budgetSection: Content[] = [
    ...
    report.budget.breakdown.length
      ? table(["Item","Faixa"], report.budget.breakdown.map((r) => [r.item, BUDGET_TIER_LABEL[r.tier]]))
  ...
  const attachmentsSection: Content[] = [
    ...
    report.attachments.length
      ? table(["Nome","Tipo","Nota"], report.attachments.map((a) => [a.name, a.kind, a.note ?? "—"]))
      : p("Nenhum anexo registrado."),
  ];

  const signSection: Content[] = [
    h1("16. Assinaturas", "sec16"),
    table(["Papel","Nome","Data","Hash digital"],
      report.signatures.map((sg) => [sg.role.toUpperCase(), sg.name ?? "________________", sg.signed_at ?? "____/____/____", sg.digital_hash ?? "—"])),
  ];

  const historySection: Content[] = [
    h1("17. Histórico de Versões", "sec17"),
    table(["Versão","Por","Quando","Alterações","Motivo"],
      report.history.map((v) => [`v${v.version}`, v.changed_by, new Date(v.changed_at).toLocaleString("pt-BR"), v.changes.join("; "), v.reason ?? "—"])),
    { text: "\n" },
  ...
  const docDefinition: TDocumentDefinitions = {
    ...
    info: { title: report.title, author: report.identification.solicitante, subject: report.identification.tipo_analise },
    content: [
      cover, toc,
      ...execSection, ...idSection, ...imagesSection, ...specialtiesSection,
      ...innovationSection, ...altSection, ...matrixSection, ...scoresSection,
      ...planSection, ...pdcaSection, ...risksSection, ...benefitsSection,
      ...scheduleSection, ...budgetSection, ...attachmentsSection, ...signSection, ...historySection,
    ],
    header: (currentPage) => currentPage === 1 ? "" : {
      margin: [40, 20, 40, 0],
      columns: [
        { text: report.title, style: "headerText" },
        { text: `Código ${report.code} · v${report.version}`, style: "headerText", alignment: "right" },
      ],
    },
    footer: (currentPage, pageCount) => ({
      margin: [40, 0, 40, 20],
      columns: [
        { text: `${report.code} · v${report.version} · ${report.identification.data}`, style: "footerText" },
    ...
    styles: {
      ...
      th: { fontSize: 9, bold: true, color: COLOR_PRIMARY, fillColor: "#f1f5f9", margin: [2,4,2,4] },
  ...
  return await new Promise<Blob>((resolve) => {
    (pdfMake.createPdf(docDefinition) as unknown as { getBlob: (cb: (b: Blob) => void) => void }).getBlob((blob) => resolve(blob));
  });
}
