...
import { FileText, FileSpreadsheet, Printer, Share2, ClipboardCopy, QrCode, Loader2 } from "lucide-react";
...
export function ReportActions({ report }: { report: TechnicalReport }) {
  ...
  async function withBusy(label: string, fn: () => Promise<void>) {
    try { setBusy(label); await fn(); }
    catch (e) { console.error(e); toast.error(`Falha ao gerar ${label}`); }
    finally { setBusy(null); }
  }

  const doPdf = () => withBusy("PDF", async () => {
    const { generateReportPdf } = await import("@/lib/reports/pdf");
    downloadBlob(await generateReportPdf(report), `${filenameBase}.pdf`);
    toast.success("PDF gerado");
  });

  const doDocx = () => withBusy("DOCX", async () => {
  ...
  const doXlsx = () => withBusy("XLSX", async () => {
  ...
  const doPrint = () => withBusy("Impressão", async () => {
    const { generateReportPdf } = await import("@/lib/reports/pdf");
  ...
  const doShare = async () => {
    ...
    try {
      if (typeof navigator !== "undefined" && "share" in navigator) {
        await (navigator as Navigator & { share: (d: ShareData) => Promise<void> }).share({ title: report.title, text });
        return;
      }
  ...
  const doQr = () => withBusy("QR", async () => {
    const target = report.qr_target_url ?? (typeof window !== "undefined" ? window.location.href : report.code);
    setQrImg(await qrDataURL(target));
    setQrOpen(true);
  });

  const btn = (key: string, icon: React.ReactNode, label: string, onClick: () => void) => (
    <Button size="sm" variant="outline" className="gap-1.5" onClick={onClick} disabled={busy !== null}>
    ...
    <>
      ...
      <Dialog open={qrOpen} onOpenChange={setQrOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>QR Code do relatório</DialogTitle></DialogHeader>
