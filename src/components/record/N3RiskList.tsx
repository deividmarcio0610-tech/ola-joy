...
export function N3RiskList({ result, selectedId, onSelect }: Props) {
  const order = result.priorizacao?.length
    ? result.priorizacao
    : result.riscos.slice().sort((a, b) => b.score - a.score).map((r) => r.id);
    ...
    <Card className="p-4 space-y-3 border-2 border-red-500/30 bg-red-50/40 dark:bg-red-950/20">
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-muted-foreground">Diagnóstico — o auditor identifica, não propõe solução.</div>
        </div>
      ...
      {ordered.length === 0 ? (
        <div className="text-sm text-muted-foreground italic">Nenhum risco visível identificado.</div>
      ) : (
            ...
            return (
              <button
                ...
                className={`w-full text-left border rounded-md p-3 transition ${
                  selected ? "border-primary ring-2 ring-primary/40 bg-primary/5" : "hover:border-primary/50 bg-background"
                }`}
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex flex-wrap gap-1 items-center">
                      ...
                      <Badge variant="outline" className="text-[10px]">{categoriaLabel(r.categoria)}</Badge>
                      {r.norma && <Badge variant="outline" className="text-[10px]">{r.norma}</Badge>}
                    ...
                    {r.risco && <div className="text-xs text-muted-foreground">Consequência: {r.risco}</div>}
                    {r.evidencia && <div className="text-[11px] text-muted-foreground italic">Evidência: {r.evidencia}</div>}
                  </div>
                  <ChevronRight className={`w-4 h-4 mt-1 ${selected ? "text-primary" : "text-muted-foreground"}`} />
                </div>
                {selected && (
                  <Button size="sm" className="w-full mt-2" onClick={(e) => { e.stopPropagation(); onSelect(r); }}>
